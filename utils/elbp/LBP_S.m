...
function pdv = LBP_S(image, pa, image_lm)
...
for i = 1:neighbors
    ...
    if (abs(x - rx) < 1e-6) && (abs(y - ry) < 1e-6)
        ...
        w1 = roundn((1 - tx) * (1 - ty),-6); % left-up
        w2 = roundn(tx * (1 - ty),-6); % right-up
        w3 = roundn((1 - tx) * ty,-6) ; % left-bottom
        % w4 = roundn(tx * ty,-6) ;
        w4 = roundn(1 - w1 - w2 - w3, -6); % right-bottom
