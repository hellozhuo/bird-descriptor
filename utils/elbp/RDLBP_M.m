...
function pdv = RDLBP_M(image, pa, image_lm)
...
pdv_whole = zeros(dy + 1, dx + 1, neighbors);

for scale_i = 0 : 1
    radius = pa.r - scale_i * pa.delta;   
    if radius < 1
        error('radius in calculating RD_LBP smaller than 1\n');
    end
    ...
    % compute pixel difference vector in scale_i
    for i = 1:neighbors
        y = spoints(i,1)+origy;
        x = spoints(i,2)+origx;
        ...
        if scale_i < 1
            ...
            pdv_whole(:, :, i) = pdv_whole(:, :, i) - N;
        end
    end
end

pdv_whole = abs(pdv_whole);
...
if(nargin > 2)
    ...
    for i = 1: pa.div(1)
        ...
        for j = 1: pa.div(2)
            ...
            pdv{i, j} = reshape(Tmp, size(Tmp, 1) * size(Tmp, 2), pa.bit);
        end
    end       
else
    rows_split = pa.div(1);
    ...
    for i = 1:rows_split
        ...
        e_h = s_h + h_region - 1;
        for j = 1:cols_split
            pdv{i, j} = zeros(h_region * w_region, pa.bit);
            s_w = w_region * (j-1) + 1;
            e_w = s_w + w_region - 1;
            Tmp = pdv_whole(s_h:pa.step:e_h, s_w:pa.step:e_w, :);   
            pdv{i,j} = reshape(Tmp, [], pa.bit);
