...
%
% para: a list of [radius1, radius2, neighbors] parameters, e.g., 
% [0, 1, 8; 2, 3, 16]
% [0, 1, 8] is equal to traditonal lbp LBP_(1, 8)
% [1, 2, 8] means the value difference between the neighbors on radius 1
% and on radius 2
%
...
function pdv = produce_pdv_rd(image, paras, image_lm)

pa = extract_pa(paras, 1);
if pa.mark < 1
    if nargin > 2
        pdv_tmp = RDLBP_S(image, pa, image_lm);
    else
        pdv_tmp = RDLBP_S(image, pa);
    end
else
    if nargin > 2
        pdv_tmp = RDLBP_M(image, pa, image_lm);
    else
        pdv_tmp = RDLBP_M(image, pa);
    end
end

scale_num = size(paras.rd, 1);
if scale_num == 1
    pdv = pdv_tmp;
else
    ...
    for i = 1: paras.div(1)
        for j = 1: paras.div(2)
            pdv{i, j} = zeros(num_points, sum(paras.rd(:, 5)));
